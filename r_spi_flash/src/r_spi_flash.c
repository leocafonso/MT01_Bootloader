...
static void sf_write_enable (uint8_t channel)
{
    ...
    uint8_t command = SF_CMD_WRITE_ENABLE;
    
    /* Initialize peripheral for SPI */
    sf_open(channel);
    
    R_RSPI_Write(channel, &command, 1, SF_PID);
...
static void sf_write_protect (uint8_t channel)
{
    ...
    command[0] = SF_CMD_WRITE_STATUS_REG;
    command[1] = SF_WP_BIT_MASK;
    R_RSPI_Write(channel, &command[0], 2, SF_PID);
...
bool R_SF_Erase (uint8_t channel, const uint32_t address, const sf_erase_sizes_t size)
{
    ...
    if (size == SF_ERASE_SECTOR)
    {
        ...
        R_RSPI_Write(channel, &command[0], 4, SF_PID);
    }
    else if(size == SF_ERASE_BULK)
    {
        /* Assign bulk erase command. */
        command[0] = SF_CMD_ERASE_BULK;

        /* bulk erase is one byte command */
        R_RSPI_Write(channel, &command[0], 1, SF_PID);
    }
    else
    {
        /* Bad command. */
...
bool R_SF_WriteData (uint8_t channel, uint32_t address, uint8_t * data, uint32_t size)
{
    ...
    while (size > 0)
    {
        ...
        R_RSPI_Write(channel, (uint8_t *)data, bytes_to_write, SF_PID);
...
bool R_SF_ReadData (uint8_t channel, const uint32_t address, uint8_t * data, const uint32_t size)
{
    ...
    /* Read data. */
    R_RSPI_Read(channel, data, size, SF_PID);  
...
static void sf_open (uint8_t channel)
{
    ...
    R_RSPI_Select(channel, FLASH_SELECTED, SF_PID);
}
...
{
    /* Deselect SPI flash. */
    R_RSPI_Deselect(channel, FLASH_SELECTED, SF_PID);
}
...
static bool sf_lock_channel (uint8_t channel)
{
    /* Lock RSPI channel for our use. */
    if (R_RSPI_Lock(channel, SF_PID) == true)
    {
        /* Lock taken, we can use channel. */
        return true;
    }
    else
    {
...
static bool sf_unlock_channel (uint8_t channel)
{
    /* Release lock on this RSPI channel. */
    if (R_RSPI_Unlock(channel, SF_PID) == true)
    {
    ...
    {
        /* We did not have permission for lock in the first place. This should not happen. */
        return false;
    }
}
...
static uint8_t sf_read_status (uint8_t channel)
{
    ...
    command = SF_CMD_READ_STATUS_REG;
    
    /* Send command. */
    R_RSPI_Write(channel, &command, 1, SF_PID);
    
    /* Read register. */
    R_RSPI_Read(channel, &command, 1, SF_PID);
